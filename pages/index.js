...
import { Calendar, Globe, Clock, Bell, PhoneCall, BarChart3 } from "lucide-react";
...
export default function Home() {
  ...
  const t = {
    ja: {
      ...
      hero: { badge: "新規予約 × リピートを自動化", title: "サロン予約、AIが24時間365日で受付", subtitle: "東京のサロンで実証済み。着信・LINE・Webからの予約を自動化し、売上と予約率を平均30%向上。", ctaPrimary: "7日間 無料トライアル", ctaSecondary: "1分でデモを見る", trust: "すでに複数のサロンに導入済み" },
      stats: { a:{label:"予約率アップ", value:"+30%"}, b:{label:"電話対応の削減", value:"-60%"}, c:{label:"導入まで", value:"24時間"} },
      features: {
        title: "サロン運営の“手間”をゼロへ",
        items: [
          { h: "24/7 自動受付", p: "営業時間外も逃さない。AI音声＆Web/LINEで予約を受付。", icon: Clock },
          { h: "Googleカレンダー連携", p: "ダブルブッキング防止、既存の運用をそのまま。", icon: Calendar },
          { h: "多言語対応", p: "日本語 / 英語 / ロシア語。インバウンドにも対応。", icon: Globe },
          { h: "自動リマインド", p: "LINE・SMS・メールで来店前にリマインド。", icon: Bell },
          { h: "通話→予約化", p: "AIが電話内容を解析し、空き枠へ即時アサイン。", icon: PhoneCall },
          { h: "レポート", p: "日次/週次で予約数・新規率・離脱率を可視化。", icon: BarChart3 }
        ]
      },
      cases: {
        title: "導入事例",
        cards: [
          { h: "表参道・カラー特化サロン", p: "夜間予約が全体の28%に。スタッフの電話対応は週あたり6時間削減。", img:"/images/case1.png" },
          { h: "横浜・メンズカット", p: "新規予約率 +33%。広告費は据え置きで売上が前月比+18%。", img:"/images/case2.png" },
          { h: "渋谷・ネイルサロン", p: "リマインド導入で無断キャンセル-41%。再来率が改善。", img:"/images/case3.png" }
        ]
      },
      pricing: { title:"料金プラン", note:"まずは7日間の無料トライアルから",
        basic:{ name:"Starter", price:"¥9,800/月", features:["予約受付（音声/LINE/Web）","Googleカレンダー連携","自動リマインド 300通/月","メールサポート"] },
        pro:{ name:"Pro", price:"¥19,800/月", badge:"おすすめ", features:["すべてのStarter機能","通話録音/要約レポート","自動リマインド 1,000通/月","優先サポート"] }
      },
      form: { title:"無料トライアルに登録", name:"お名前", salon:"サロン名", phone:"電話番号", email:"メールアドレス", submit:"登録してはじめる", small:"登録は1分。クレジットカード不要。" },
      footer: { made:"日本のサロンのために設計", rights:"© 2025 SalonAI" }
    },
    en: {
      nav: { features: "Features", pricing: "Pricing", cases: "Case Studies", signup: "Start Free" },
      hero: { badge: "Automate new & repeat bookings", title: "AI takes your salon bookings 24/7", subtitle: "Proven in Tokyo salons. Turn calls, LINE and Web into confirmed bookings, boosting revenue and conversion by 30% on average.", ctaPrimary: "Start 7‑day free trial", ctaSecondary: "Watch 1‑min demo", trust: "Already live in multiple salons" },
      stats: { a:{label:"Conversion lift", value:"+30%"}, b:{label:"Less phone time", value:"-60%"}, c:{label:"Go‑live in", value:"24h"} },
      features: {
        title: "Zero admin, more bookings",
        items: [
          { h: "24/7 intake", p: "Don’t miss after‑hours. Voice, Web & LINE auto‑booking.", icon: Clock },
          { h: "Google Calendar", p: "No double‑booking. Keep your current workflow.", icon: Calendar },
          { h: "Multilingual", p: "Japanese / English / Russian. Great for inbound guests.", icon: Globe },
          { h: "Auto‑reminders", p: "LINE/SMS/Email to cut no‑shows.", icon: Bell },
          { h: "Calls → bookings", p: "AI parses calls and assigns to free slots instantly.", icon: PhoneCall },
          { h: "Reports", p: "Daily/weekly metrics: bookings, new ratio, drop‑offs.", icon: BarChart3 }
        ]
      },
      cases: {
        title: "Case Studies",
        cards: [
          { h: "Omotesando – Color‑first salon", p: "28% of bookings now at night. ~6h/week phone time saved.", img:"/images/case1.png" },
          { h: "Yokohama – Men’s cuts", p: "+33% new bookings. +18% MoM revenue with same ad spend.", img:"/images/case2.png" },
          { h: "Shibuya – Nail studio", p: "No‑shows down 41% via reminders. Better repeat rates.", img:"/images/case3.png" }
        ]
      },
      pricing: { title:"Pricing", note:"Start with a 7‑day free trial",
        basic:{ name:"Starter", price:"¥9,800/mo", features:["Bookings (Voice/LINE/Web)","Google Calendar sync","Auto reminders 300/mo","Email support"] },
        pro:{ name:"Pro", price:"¥19,800/mo", badge:"Most popular", features:["All Starter features","Call recording & summaries","Auto reminders 1,000/mo","Priority support"] }
      },
      form: { title:"Register for free trial", name:"Your name", salon:"Salon name", phone:"Phone", email:"Email", submit:"Create my trial", small:"Takes 1 minute. No credit card required." },
    ...
    <div className="min-h-screen bg-hero-gradient text-white">
      ...
      <header className="mx-auto max-w-7xl px-4 pt-16 pb-20 grid md:grid-cols-2 items-center gap-10">
        <div>
          ...
          <h1 className="mt-5 text-4xl md:text-6xl font-semibold tracking-tight leading-tight">{t.hero.title}</h1>
        ...
        <div className="relative">
          ...
          <div className="relative max-w-lg mx-auto rounded-3xl border border-white/15 bg-white/5 shadow-soft p-4 md:p-5">
            ...
            <img src="/images/demo.png" alt="Demo preview" className="rounded-2xl w-full h-auto" />
          </div>
        </div>
      </header>
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-6">
            {t.features.items.map((f, i) => (
              <div key={i} className="group rounded-2xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition">
                <div className="h-12 w-12 flex items-center justify-center rounded-xl bg-white/10 border border-white/20 text-brand-400 mb-4">
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-6">
            {t.cases.cards.map((c, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-gradient-to-br from-fuchsia-600/30 to-cyan-400/20 p-6">
                <img src={c.img} alt={c.h} className="rounded-xl w-full h-auto mb-4 border border-white/20" />
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-8">
              ...
              <ul className="mt-6 space-y-3 text-sm text-white/80">
                {t.pricing.basic.features.map((f, i) => (
                  <li key={i} className="flex gap-2"><span className="mt-1 h-2 w-2 rounded-full bg-white/70" />{f}</li>
                ))}
              </ul>
            ...
            <div className="relative rounded-3xl border border-white/20 bg-white/10 p-8 shadow-soft">
              ...
              <ul className="mt-6 space-y-3 text-sm text-white">
                {t.pricing.pro.features.map((f, i) => (
                  <li key={i} className="flex gap-2"><span className="mt-1 h-2 w-2 rounded-full bg-white" />{f}</li>
                ))}
              </ul>
          ...
          <div className="rounded-3xl border border-white/15 bg-white/10 p-8 md:p-12 shadow-soft">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="border border-dashed border-white/20 rounded-2xl p-6">
                ...
                <img src="/images/qr.png" alt="QR placeholder" className="mt-3 rounded-xl w-full h-auto border border-white/20" />
